...
double RL_torque_command_left  = 0;    
double RL_torque_command_right = 0;   
...
int sensor_type = 1;    //// 0 for using IMU and 1 for using encoder  
int l_ctl_dir   = -1; 
int r_ctl_dir   = -1; 
//**************************
...
void loop() {
  ...
  if (current_time - previous_time > Tinterval_ctrl_micros) { 
    ...
    RL_torque_command_left  = assistive_ratio * L_CMD_serial * l_ctl_dir;     
    RL_torque_command_right = assistive_ratio * R_CMD_serial * r_ctl_dir;       

    M1_torque_command = RL_torque_command_left;     
    M2_torque_command = RL_torque_command_right;     
...
void RealEncoder() {
  f_LTAVx = LTAVx.addSample(m1.spe/PI*180);    
  f_RTAVx = RTAVx.addSample(m2.spe/PI*180);    

  L_IMUX_int = Serial_Com.float_to_uint(m1.pos/PI*180, -180, 180, 16);   
  R_IMUX_int = Serial_Com.float_to_uint(m2.pos/PI*180, -180, 180, 16);   
...
void M1_Position_Control_Example() {
  ...
  kp = 30;    //max 450 min 0
  kd = 1.5;   //max 5 min 0
  t_ff = M1_torque_command;   //dont change this
  m1.send_cmd(p_des, v_des, kp, kd, t_ff);
  receive_CAN_data();
}
...
void M2_Position_Control_Example() {
  ...
  kp = 30;    //max 450 min 0
  kd = 1.5;   //max 5 min 0
  t_ff = M2_torque_command;   //dont change this
  m2.send_cmd(p_des, v_des, kp, kd, t_ff);
  receive_CAN_data();
}
...
void Transmit_ble_Data(){
  ...
  L_motor_torque_desired = RL_torque_command_left * 100;
  R_motor_torque_desired = RL_torque_command_right * 100;

  L_pos_int_d = 0.0 * value_scale_ratio;              
  L_pos_int_a = m1.pos * value_scale_ratio;              
  L_vel_int_d = 0.0 * value_scale_ratio;      
  L_vel_int_a = m1.spe * value_scale_ratio;       
  R_pos_int_d = 0.0 * value_scale_ratio;              
  R_pos_int_a = m2.pos * value_scale_ratio;              
  R_vel_int_d = 0.0 * value_scale_ratio;      
  R_vel_int_a = m2.spe * value_scale_ratio;          
