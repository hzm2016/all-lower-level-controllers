...
void Serial_Com::READREF()  
{
  if (SERIAL_Isra.available() >= SerialDataRef_Length)
  {
   
    SerialData[0] = SERIAL_Isra.read();   
    if (SerialData[0] == Header[0])  
    {
      SerialData[1] = SERIAL_Isra.read();
      if (SerialData[1] == Header[1]) 
      {
        SerialData[2] = SERIAL_Isra.read();   
        if (SerialData[2] == Header[2]) {
          SERIAL_Isra.readBytes(&SerialData[3], SerialDataRef_Length - 3);
          for (int i = 0; i <= SerialDataRef_Length; i++) 
          {
            SerialDataRef[i] = SerialData[i];   
          }
        }
      }
    }
